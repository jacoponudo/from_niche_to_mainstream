...
ggplot(reddit_size_vs_interaction, aes(x = post_size_class, y = interaction)) +
...
reddit_size_vs_interaction <- data %>%
  ...
  mutate(
    post_size_class = cut(
      post_size,
      breaks = quantile(post_size, probs = seq(0, 1, length.out = 21), na.rm = TRUE),
      include.lowest = TRUE,
      labels = paste0(
        "(", 
        round(quantile(post_size, probs = seq(0, 1, length.out = 21), na.rm = TRUE)[-length(quantile(post_size, probs = seq(0, 1, length.out = 21), na.rm = TRUE))], 2), 
        ", ", 
        round(quantile(post_size, probs = seq(0, 1, length.out = 21), na.rm = TRUE)[-1], 2), 
        "]"
      )
    )
  )
...
ggplot(mean_interactions, aes(x = post_size_class, y = mean_interaction)) +
  geom_line(color = "blue", group = 1) +
  geom_point(color = "blue", size = 2) +
  labs(x = "Post Size Class", y = "Lambda", title = "Mean Interaction by Post Size Class") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
